trigger CaseTransfer on Case (after update) {
  ...
  //Workflow rules that cause a field update cause triggers to fire again. 
  //In order to prevent this, creating a static variable in a Class that will
  //prevent logic from happening a second time
   
  if (!RecursiveTrigger.hasAlreadyFiredTrigger()){
	  ...
	  RecursiveTrigger.setAlreadyFiredTrigger();
